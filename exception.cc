...
ExceptionHandler(ExceptionType which)
{
	...
	case SyscallException :
		...
		//printf("ch = $d\n", ch);
		if (!machine->ReadMem(bufferAdd, 1, &i)) return;
		int i = 0;
		while (ch != 0)
		{
			filename[i] = *ch;
    ...
    space = new AddrSpace(executable);
		Thread * t = new Thread ("User Exec");
    t->space = space;

		int processID = 1;  //MUST DECLARE getID(), current is WRONG!
    ...
    ASSERT(FALSE);			// machine->Run never returns;
		...
		case SC_Exit :
		{
			//for multiprocessing, clean up exited process memory before exiting
		...
		case SC_Yield :
		{
			//might need updating for multiprocessing
			currentThread->Yield();
			break;
		}
